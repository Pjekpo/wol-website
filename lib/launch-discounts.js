...
const DISCOUNT_STORAGE_ENV = [
  ["SUPABASE_URL", SUPABASE_URL],
  ["SUPABASE_SERVICE_ROLE_KEY", SUPABASE_SERVICE_ROLE_KEY]
];
const LAUNCH_DISCOUNT_ENV = [
  ...DISCOUNT_STORAGE_ENV,
...
async function getOrCreateStripeCustomer({ email, source = "waitlist", reward = "10% off" }) {
  const stripe = getStripe();
  const existingCustomers = await stripe.customers.list({
    email,
    limit: 1
  });
  const existingCustomer = existingCustomers.data.find(function (customer) {
    return normalizeEmail(customer.email) === normalizeEmail(email);
  });
  ...
  const createdCustomer = await stripe.customers.create({
    ...
    metadata: {
      source,
      reward,
      purpose: "launch_discount_waitlist"
    }
  });
...
export function discountStorageConfigured() {
  return getMissingDiscountStorageEnvVars().length === 0;
}

export function launchDiscountDispatchConfigured() {
  return getMissingLaunchDiscountEnvVars().length === 0;
}

export function getMissingDiscountStorageEnvVars() {
  return DISCOUNT_STORAGE_ENV
...
export function getMissingLaunchDiscountEnvVars() {
  return LAUNCH_DISCOUNT_ENV
    .filter(function ([, value]) {
      return !value;
    })
    .map(function ([name]) {
...
async function createStripeCoupon() {
  ...
  const coupon = await stripe.coupons.create({
    percent_off: LAUNCH_DISCOUNT_PERCENT,
    duration: "once",
    name: `WOL ${LAUNCH_DISCOUNT_PERCENT}% launch discount`,
    metadata: {
      purpose: "launch_discount"
    },
    ...(LAUNCH_DISCOUNT_EXPIRY_ISO
      ? (() => {
          const redeemBy = Math.floor(new Date(LAUNCH_DISCOUNT_EXPIRY_ISO).getTime() / 1000);
          return Number.isFinite(redeemBy) && redeemBy > 0 ? { redeem_by: redeemBy } : {};
        })()
      : {})
  });
...
async function createPromotionCode({ email, couponId, stripeCustomerId, source = "waitlist", reward = "10% off" }) {
  ...
  const promotion = await stripe.promotionCodes.create({
    ...
    ...(LAUNCH_DISCOUNT_EXPIRY_ISO
      ? (() => {
          ...
          return Number.isFinite(expiresAt) && expiresAt > 0 ? { expires_at: expiresAt } : {};
        })()
      : {})
  });

  if (!promotion.id || !promotion.code) {
...
export async function claimDiscountAndSendEmail({ email, source = "waitlist", reward = "10% off" }) {
  ...
  try {
    ...
    const stripeCustomerId = await getOrCreateStripeCustomer({
      ...
      source: workingClaim.source || source,
      reward: workingClaim.reward || reward
    });
